...
class NovelApp:
    ...
    def generate_formal_characters(self, *, book_id: str) -> str:
        ...
        def task(store: SQLiteStore, handle: RunHandle) -> None:
            master = self._build_master(store)
            with EventBus(run_id=run_id, store=store, cancel_event=handle.cancel_event):
                master.generate_characters_for_book(book_id=book_id, run_id=run_id, mode="formal")
    ...
    def generate_formal_chapter_plans(self, *, book_id: str) -> str:
        ...
        def task(store: SQLiteStore, handle: RunHandle) -> None:
            master = self._build_master(store)
            with EventBus(run_id=run_id, store=store, cancel_event=handle.cancel_event):
                master.generate_chapter_roadmap_for_book(book_id=book_id, run_id=run_id, mode="formal")
...
.modal{position:fixed;inset:0;background:rgba(3,6,14,.76);display:none;align-items:center;justify-content:center;z-index:20;padding:28px}.modal-card{width:min(920px,calc(100vw - 56px));max-height:calc(100vh - 56px);display:flex;flex-direction:column;background:#151a23;border:1px solid #303a52;border-radius:16px;box-shadow:0 24px 90px rgba(0,0,0,.52);overflow:hidden}.modal-head{padding:18px 22px;border-bottom:1px solid #263047;background:linear-gradient(135deg,#171f31,#131824)}.modal-title{font-size:17px;color:#eef2ff;margin-bottom:4px}.modal-desc{font-size:12px;color:#8997b8;line-height:1.6}.modal-body{padding:20px 22px;overflow:auto}.modal-section{border:1px solid #263047;background:#111722;border-radius:12px;padding:16px;margin-bottom:14px}.modal-section-title{font-size:12px;color:#8ea1d8;text-transform:uppercase;letter-spacing:.08em;margin-bottom:12px}.field-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.field{display:flex;flex-direction:column;gap:6px}.field.full{grid-column:1/-1}.field label{color:#dce3fb;font-size:12px}.field textarea,.field input,.field select{width:100%;font-size:13px}.field textarea{min-height:132px;resize:vertical}.field input{height:34px}.field-help{color:#7f8aa3;font-size:11px;line-height:1.55}.config-placeholder{border:1px dashed #34405f;border-radius:10px;padding:12px;color:#7f8aa3;font-size:12px;line-height:1.7;background:#0f141f}.modal-actions{display:flex;justify-content:flex-end;gap:8px;padding:14px 22px;border-top:1px solid #263047;background:#111722}
...
<div id='hdr'><h1>Novel Flow</h1><select id='modeSel' onchange='changeMode()'><option value='formal'>正式模式</option><option value='test'>测试模式</option></select><select id='novelSel' onchange='selectNovel(this.value)'><option value=''>选择小说</option></select><span id='stage-pill'>未开始</span><button id='btnNew' onclick='openNewNovelDialog()'>新建小说</button><button id='btnCharacters' onclick='generateCharacters()'>生成角色</button><button id='btnRoadmap' onclick='generateChapterPlans()'>生成章节规划</button><button id='btnContinue' onclick='continueFormal()'>写下一章</button><button id='btnBlueprint' onclick='testBlueprint()' style='display:none'>测试大纲</button><button id='btnWrite' onclick='testWrite()' style='display:none'>测试写章节</button><button id='btnCritique' onclick='testCritique()' style='display:none'>测试评价</button><button id='btnPatch' onclick='testPatch()' style='display:none'>测试修改</button><button id='btnStop' class='ghost' onclick='stopCurrentRun()' style='display:none'>停止运行</button><button class='danger' onclick='deleteNovel()'>删除小说</button></div>
<div id='newNovelModal' class='modal'><div class='modal-card'><div class='modal-head'><div class='modal-title'>新建小说</div><div class='modal-desc'>填写题材和可选风格。风格不填时，系统会根据题材自行判断；后续新增配置会放在这个表单里。</div></div><div class='modal-body'><div class='modal-section'><div class='modal-section-title'>基础信息</div><div class='field-grid'><div class='field full'><label>题材/需求</label><textarea id='newQueryInput' placeholder='例如：都市情感反转，女主发现丈夫隐藏身份后反击'></textarea><div class='field-help'>写清题材、主角处境、核心冲突或你想看的关键局面。</div></div><div class='field full'><label>风格要求（可留空）</label><input id='newStyleInput' placeholder='例如：古言权谋、轻喜剧、短篇悬疑、第三人称群像；留空则由系统自行判断' /><div class='field-help'>这里是硬约束。只在你明确填写时生效，不再默认知乎体、女频或固定人称。</div></div></div></div><div class='modal-section'><div class='modal-section-title'>扩展配置</div><div class='config-placeholder'>后续可以在这里增加目标体量、章节数、叙事视角、禁用元素、参考卡片范围等配置。当前版本先使用系统默认决策。</div></div></div><div class='modal-actions'><button class='ghost' onclick='closeNewNovelDialog()'>取消</button><button onclick='startFormalFromDialog()'>开始生成</button></div></div></div>
...
async function refreshPendingRun(){if(!pendingRunId)return;const d=await api(`/api/run?mode=${mode}&run_id=${pendingRunId}`);stagePill.textContent=stageText(d.stage||'writing');if(d.current_book_id){bookId=d.current_book_id;pendingRunId='';novelSel.value=bookId;return refreshNovel();}const running=d.is_running!==false;runsCache=[{run_id:pendingRunId,is_running:running,stage:d.stage,updated_at:d.updated_at||new Date().toISOString(),pending_message:running?'运行中，等待模型返回更多内容。':'运行已结束，查看下方最新事件。'}];expandedRuns.add(pendingRunId);await renderRuns({[pendingRunId]:d});updateStopButton();}
...
function buildStreamItems(runId,evts){const groups=[],byId={};let active=null,seq=0;function ensureGroup(callId,ts){const key=callId||`legacy_${++seq}`;if(byId[key])return byId[key];const group={key,text:'',reply:'',sortTs:ts||'',done:false};byId[key]=group;groups.push(group);return group;}evts.forEach(e=>{const payload=e.payload||{},callId=payload.call_id||'';if(e.event_type==='llm_prompt'){active=ensureGroup(callId||`prompt_${e.id||++seq}`,e.ts);active.prompt=payload.preview||'';}else if(e.event_type==='llm_stream'){const group=callId?ensureGroup(callId,e.ts):(active&&!active.done?active:ensureGroup(`stream_${e.id||++seq}`,e.ts));group.text+=payload.preview||'';group.sortTs=e.ts||group.sortTs;}else if(e.event_type==='llm_reply'){const group=callId?ensureGroup(callId,e.ts):active;if(group){group.reply=payload.preview||'';group.done=true;group.sortTs=e.ts||group.sortTs;}}});return groups.filter(group=>group.text||group.reply).map((group,index)=>({key:`${runId}:stream:${group.key}`,title:`DoubaoLLM · 流式输出 #${index+1}`,kind:'plain',text:group.text||group.reply||'',sortTs:group.sortTs||''}));}
async function renderRuns(pref){if(!runsCache.length){evs.innerHTML="<div class='empty'>当前还没有运行记录</div>";return;}const cache=pref||{};for(const r of runsCache){if(expandedRuns.has(r.run_id)&&!cache[r.run_id])cache[r.run_id]=await api(`/api/run?mode=${mode}&run_id=${r.run_id}`);}let h='';runsCache.forEach(r=>{const ex=expandedRuns.has(r.run_id),d=cache[r.run_id],outs=(d&&d.outputs)||[],evts=(d&&d.events)||[];const streamItems=buildStreamItems(r.run_id,evts);const normalEvents=evts.filter(e=>!['llm_stream','llm_prompt','llm_reply'].includes(e.event_type));const items=[];if(r.pending_message)items.push({key:`${r.run_id}:pending`,title:'系统提示',kind:'plain',text:r.pending_message,sortTs:r.updated_at||''});streamItems.forEach(item=>items.push(item));outs.forEach(o=>items.push({key:`${r.run_id}:out:${o.id}`,title:`${esc(o.agent)} · ${esc(o.title)}`,kind:'output',outputType:o.output_type,rawPayload:o.payload,sortTs:o.created_at||''}));normalEvents.forEach(e=>items.push({key:`${r.run_id}:evt:${e.id}`,title:`${esc(e.agent||'System')} · ${esc(e.title||'')}`,kind:'event',eventType:e.event_type,rawPayload:e.payload,sortTs:e.ts||''}));items.sort((a,b)=>String(a.sortTs).localeCompare(String(b.sortTs)));h+=`<div class='run'><div class='head' onclick="toggleRun('${r.run_id}')"><span class='tag'>${esc(stageText(r.stage))}</span>${r.is_running?"<span class='tag live'>运行中</span>":''}${r.cancel_requested?"<span class='tag stop'>停止中</span>":''}<span>${esc(r.run_id)}</span><span class='ts'>${esc(shortTs(r.updated_at))}</span></div>`;if(ex){h+="<div class='body'>";if(r.is_running)h+=`<div style='margin-bottom:8px'><button class='ghost' onclick="event.stopPropagation();stopRun('${r.run_id}')">停止并删除这次运行</button></div>`;if(!items.length)h+="<div class='payload'>当前还没有过程数据。</div>";items.forEach((item,index)=>{const isLatest=index===items.length-1;const isOpen=(item.key in boxStates)?boxStates[item.key]:isLatest;h+=boxHtml(item.key,item.title,renderItemPayload(item),isOpen);});h+='</div>';}h+='</div>';});evs.innerHTML=h;}
...
function openNewNovelDialog(){newNovelModal.style.display='flex';newQueryInput.focus();}
function closeNewNovelDialog(){newNovelModal.style.display='none';}
async function startFormalFromDialog(){const q=newQueryInput.value.trim();if(!q)return alert('请输入新建小说的题材/需求。');const style=newStyleInput.value.trim();closeNewNovelDialog();await startFormal(q,style);}
async function startFormal(q,style=''){const r=await api('/api/novels/start',{method:'POST',body:JSON.stringify({query:q,style_request:style})});if(!ensureOk(r))return;pendingRunId=r.run_id||'';expandedRuns=new Set(pendingRunId?[pendingRunId]:[]);boxStates={};stagePill.textContent='启动中';runsCache=pendingRunId?[{run_id:pendingRunId,is_running:true,stage:'planning',updated_at:new Date().toISOString(),pending_message:style?`大纲任务已启动，题材：${q}；风格：${style}`:'大纲任务已启动，正在准备请求模型。'}]:[];await renderRuns();updateStopButton();}
async function generateCharacters(){if(!bookId)return alert('请先选择一部小说。');const r=await api('/api/novels/generate_characters',{method:'POST',body:JSON.stringify({book_id:bookId})});if(!ensureOk(r))return;pendingRunId=r.run_id||'';expandedRuns.add(pendingRunId);boxStates={};runsCache=[{run_id:pendingRunId,is_running:true,stage:'planning',updated_at:new Date().toISOString(),pending_message:'角色生成任务已启动。'},...runsCache.filter(x=>x.run_id!==pendingRunId)];await renderRuns();updateStopButton();}
async function generateChapterPlans(){if(!bookId)return alert('请先选择一部小说。');const r=await api('/api/novels/generate_chapter_plans',{method:'POST',body:JSON.stringify({book_id:bookId})});if(!ensureOk(r))return;pendingRunId=r.run_id||'';expandedRuns.add(pendingRunId);boxStates={};runsCache=[{run_id:pendingRunId,is_running:true,stage:'planning',updated_at:new Date().toISOString(),pending_message:'章节规划任务已启动。'},...runsCache.filter(x=>x.run_id!==pendingRunId)];await renderRuns();updateStopButton();}
