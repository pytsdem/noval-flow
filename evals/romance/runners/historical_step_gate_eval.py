...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run upstream step evals and chapter-ready gate checks on exported historical cases.")
    ...
    parser.add_argument("--reports-root", default="evals/romance/reports", help="Directory where step-eval reports are written.")
    parser.add_argument("--eval-summary", default="", help="Optional romance eval summary.json used to enrich step diagnostics before gating.")
    return parser


def main() -> None:
    args = build_parser().parse_args()
    runner = StepEvalRunner(reports_root=args.reports_root)
    summary = runner.run(
        case_dir=args.cases,
        label=args.label,
